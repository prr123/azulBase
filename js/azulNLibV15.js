...
//				console.log(`key: ${key}, val: ${value}`);
	...
	addGrid(gridRobj) {
		...
		gridDiv.style.gridTemplateColumns = gridRobj.cols;
//		gridDiv.gridTemplateRows = gridRobj.rows;
		gridDiv.ncol = gridRobj.ncol;
		gridDiv.nrow = gridRobj.nrow;

		Object.assign(gridDiv.style, gridRobj.style);

		let geStyl = {};
		if (gridRobj.hasOwnProperty('elStylObj')) {geStyl = gridRobj.elStylObj;}

		if (Object.hasOwn(gridRobj,'elStyle')) {geStyl = gridRobj.elStyle;}
...
//    	gridDiv.style.gridTemplateRows = repStr;
    	...
    	for (let irow = 0; irow< gridRobj.nrow; irow++) {
			gridEls[irow] = [];
        	for (let icol = 0; icol<gridRobj.ncol; icol++) {
