...
// V14: add class azulBlog
//      add isApha etc
// V14a: add image
// V15: add replace function
//      enhance addGrid
// V16: add basic stylesheet
//
...
//		const st = document.styleSheets.length;
		const nsheet = new CSSStyleSheet();
		styleEl.sheet = nsheet;

//        styleSheet.insertRule('* { margin: 0; padding: 0; font-family: Calibri; list-style: none; text-decoration:none;}');
		if (docObj.hasOwnProperty('rules')) {
			for (let ir=0; ir < docObj.rules.length; ir++) {
				let rul = docObj.rules[ir];
				nsheet.insertRule(rul, ir);
			}
		} else {
	        nsheet.insertRule('* { margin: 0; padding: 0; font-family: Calibri; font-size: 20px; text-decoration:none;}');
		}


		document.adoptedStyleSheets.push(nsheet);

		this.styleSheet = nsheet;
*/
...
//     	Object.assign(divMain,pgObj.mainDiv);
	...
	rplSect(newDiv) {
//		console.log('replace: ' + newDiv.tagName);
//		if (newDiv.tagName != 'DIV') {throw new Error("newDiv is not a div!");}
        const list = azul.Main;
...
//      console.log('elObj: ', + elObj);
		if (elObj.typ === undefined) {throw new Error("obj has no typ!");}
...
//				console.log(`key: ${key}, val: ${value}`);
	...
	addImg(imgObj) {
		const im = new Image();
		if (imgObj.height !== undefined) {im.height = imgObj.height;}
		if (imgObj.width !== undefined) {im.width = imgObj.width;}
		im.src = imgObj.src;

		if (imgObj.hasOwnProperty('stylObj')) {
			if (imObj.style === undefined) {im.style = {};}
			Object.assign(im.style,imgObj.stylObj);}

		if (Object.hasOwn(imgObj,'style')) {Object.assign(im.style,imgObj.style);}

		if (Object.hasOwn(imgObj,'Nam')) {this[imgObj.Nam] = im;}
		if (Object.hasOwn(imgObj,'alt')) {im.alt = imgObj.alt;}
		if (Object.hasOwn(imgObj,'load')) {im.loading = imgObj.load;}

		if (Object.hasOwn(imgObj,'evlist')) {
			for (const [key, value] of Object.entries(imgObj.evlist)) {
//				console.log(`key: ${key}, val: ${value}`);
				im.addEventListener(key,value);
			}
		}
		if (imgObj.hasOwnProperty('parent')) {imgObj.parent.appendChild(im);}
		return im;
	},
	...
	addGrid(gridDefObj) {
		...
		if (grifDefObj.cols) {
			gridDiv.style.gridTemplateColumns = gridDefObj.cols;
		} else {
    		gridDiv.style.gridTemplateColumns = 'repeat(' + gridDefObj.ncol + ', 1fr)';
		}
		if (grifDefObj.rows) {
			gridDiv.style.gridTemplateRows = gridDefObj.rows;
		} else {
    		gridDiv.style.gridTemplateRows = 'repeat(' + gridDefObj.nrow + ', 1fr)';
		}

		gridDiv.ncol = gridDefObj.ncol;
		gridDiv.nrow = gridDefObj.nrow;

		Object.assign(gridDiv.style, gridDefObj.style);

		let geStyl = {};
		if (gridDefObj.hasOwnProperty('elStylObj')) {geStyl = gridDefObj.elStylObj;}

		if (Object.hasOwn(gridDefObj,'elStyle')) {geStyl = gridDefObj.elStyle;}

		let gridEls = [];

    	for (let irow = 0; irow< gridDefObj.nrow; irow++) {
			gridEls[irow] = [];
        	for (let icol = 0; icol<gridDefObj.ncol; icol++) {
...
class AzulBlog {
    ...
    render() {
        let rendiv = this.obj.rend(this.blogDiv);
        this.fbut.addEventListener('click', function() {azul.butfunc(rendiv, azul.docbody);}, false);
    }
}
